...
class CostRecourseScoringApprox():
    ...
    def compute_score(self) -> float:
        ...
        total_score = np.median(delta_cf_cost)
...
class CostResourceScoring(Scoring):
    def __init__(self, clf, X_test: np.ndarray,
                 y_sensitive_test: np.ndarray,
    ...
    def compute_score(self, X_train: np.ndarray, y_train: np.ndarray, n_itr: int = 1) -> float:
        ...
        for _ in range(n_itr):
            ...
            total_score = [__cost(delta_cf) for delta_cf in Delta_CF]
...
class MemoryCostResourceScoring(CostResourceScoring):
    def compute_cf_batch(self, clf, X_test, y_target_label, X_train, y_train):
...
class ProtoCostResourceScoring(CostResourceScoring):
    def compute_cf_batch(self, clf, X_test, y_target_label, X_train, y_train):
...
class WachterCostResourceScoring(CostResourceScoring):
    def compute_cf_batch(self, clf, X_test, y_target_label, X_train, y_train):
